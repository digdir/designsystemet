...
Sjølv om det kan verke freistande å lage ein webkomponentar av alt, til dømes ein eigen knapp:
...
I tillegg får vi dårlegare yting, sidan vi blir avhengige av JavaScript, og du som utviklar kan ikkje lenger nytte HTML-dokumentasjon for å forstå koden.

Kort sagt: Mange komponentar treng - og bør - rett og slett ikkje vere webkomponentar.
Moderne CSS med klassenamn og data-attributt, gir i mange tilfelle både betre fleksibilitet og betre yting.

## Kva komponentar vil bli webkomponentar?

Komponentar som i dag nyttar React for å implementere logikk, vil bli gjorde tilgjengelege som webkomponentar. 
Til dømes vil `<ds-field>` kople saman skjemafelt med `<label>` og feilmeldingar, og 
`<ds-tabs>` vil handtere tab-navigasjon og tilhøyrande innhald.

Andre komponentar, som `Popover` og `Dialog`, treng ikkje bli webkomponentar, fordi web standarden allereie
tilbyr `events` eller moderene [`command`-attributtar](https://developer.chrome.com/blog/command-and-commandfor) (med tilhøyrende [polyfill (github.com)](https://github.com/keithamus/invokers-polyfill)) som vi kan nytte for å hjelpe med opning og lukking, og plassering.

Uavhengig av teknisk løysing vil alle komponentane vere baserte på rein HTML,
fungere på tvers av teknologiar og ha det same API-et:
...
Vi vil fortløpande leggje ut eksperimentelle komponentar i pakken @digdir/designsystemet-native, slik at du raskt kan teste og kome med tilbakemeldingar.
