...
If you want SVG graphics to use the current text colour by default, you can use currentColor. Remove any hard-coded colours from the SVG and set the fill colour to currentColor. Depending on the graphic, you may need to do the same for stroke.
```CSS
svg {
    fill: currentColor;
}
...
- **SVG has the wrong size**
  ...
  - Check that `width` and `height` are defined on the `<svg>` element or set through CSS
