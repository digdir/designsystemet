...
`focusgroup` er en nettleser- attributt som muliggjør tastaturnavigasjon for komponenter som verktøylinjer og menyer.
Den støttes av Chrome 150 og nyere, og polyfilles automatisk av Designsystemet for nettlesere som ikke støtter den ennå.
[Se Open UI-forklaringen &rarr;](https://open-ui.org/components/scoped-focusgroup.explainer/)
...
| --- | --- | --- |
| `toolbar`, `tablist`, `radiogroup`, `listbox`, `menu` eller `menubar` | Ja | Setter standard ARIA-roller og [standardverdier for de øvrige tokenene](https://open-ui.org/components/scoped-focusgroup.explainer/#focusgroup-tokens). |
...
**Merk:** Foretrekk å bruke [Tabs](/no/components/docs/tabs) fremfor `focusgroup="tablist"` i Designsystemet, ettersom [Tabs](/no/components/docs/tabs) allerede utvider dette mønsteret med fane-aktivering, automatisk kobling av faner til paneler osv.
