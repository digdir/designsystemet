...
Dette er nyttig når du for eksempel vil gjøre en hel tabellrad klikkbar, eller et kort klikkbart uten at hele kortets innhold blir lenketeksten for skjermlesere.

Baserer seg på disse:
...
Bruk `data-clickdelegatefor` når du har et interaktivt element som `a`, `button` eller `input[type="checkbox"]`, men du ønsker å utvide klikkflaten. Dette er spesielt nyttig for tabellrader (`<tr>`) og kort (`<div>`).
