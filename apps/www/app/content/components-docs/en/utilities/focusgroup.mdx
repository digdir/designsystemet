...
It is supported by Chrome 150 and later, and automatically polyfilled by Designsystemet for browsers that don't yet support it yet.
[See the Open UI explainer &rarr;](https://open-ui.org/components/scoped-focusgroup.explainer/)
...
| --- | --- | --- |
| `toolbar`, `tablist`, `radiogroup`, `listbox`, `menu` or `menubar` | Yes | Sets default ARIA roles and [defaults for the rest of the tokens](https://open-ui.org/components/scoped-focusgroup.explainer/#focusgroup-tokens). |
...
If the focusable elements do not have an explicit `role`, `focusgroup` will automatically setup ARIA roles for you, based on the behavior token.
