---
search_terms: action, cta, submit, save, cancel, confirm
---
...
A link can look like a button. Still use `<a>` when the element navigates to a new page or URL. Use `<button>` when the element performs an action on the page.

First consider whether a regular link is clearer and more predictable for the user.
...
What the element **does**, not how it looks, determines whether you should use `<button>` or `<a>`.

Links can have different visual appearances depending on the context. For example, links in a navigation menu can be without underline or icons, as long as it is clear that they are interactive and belong to the navigation.

A link can also look like a button when it represents a clear navigation action. It should still be implemented as `<a>`, [see the example above](#button-styled-as-a-link).

#### Log in

“Log in” can look like a button regardless of which HTML element you use.
Use a `<button>` element when “Log in” actually logs the user in, for example by submitting a login form. Use an `<a>` element when “Log in” sends the user to a login page or an external authentication service.
...
## Text
All buttons on the same page must have unique button text (unless multiple buttons perform the same action).
...
### Be consistent
Use the same terms in button labels that are used elsewhere in your service. Read about the [terms we recommend](/en/best-practices/content-work/terms).
