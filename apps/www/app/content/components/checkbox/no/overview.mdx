...
**Unngå `Checkbox` når**
- brukeren skal velge kun ett alternativ, bruk heller en [`Select`](/no/components/select/overview), [`Radio`](/no/components/radio/overview) eller en enkel [`Switch`](/no/components/switch/overview)
- brukeren skal velge mellom flere enn 7 svaralternativer, bruk heller en [`Select`](/no/components/select/overview) eller [`Suggestion`](/no/components/suggestion/overview)
...
De aller fleste ganger brukes `Checkbox` i grupper, der brukeren kan velge flere alternativer.
`<fieldset>` som er rundt gruppen burde ha en tittel som forklarer hva valgene handler om.
...
Ikke alle vil forstå hvorfor de ikke får til å endre innholdet i feltet.

<Story story="ReadOnly" />

### Disabled

Vi bør unngå at `Checkbox` er deaktivert (`disabled`) fordi det kan være vanskelig å oppfatte.
Noen brukere vil ikke forstå hva valget sier eller hvorfor det ikke er klikkbart.
Hvis en `Checkbox` eller gruppe med `Checkbox` ikke er relevant, bør du helst fjerne valgene fremfor å deaktivere dem.
Tilby gjerne brukeren informasjon om hvorfor valgene ikke er tilgjengelige.
...
Bruk et aktivt og positivt språk. Tenk på innholdet som en samtale.
Når `<legend>` er en setning, kan du skrive den i du-form og svaralternativene (`<label>`) i jeg-form. `<label>` skal alltid ha stor forbokstav.
