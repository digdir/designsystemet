...
**Use `Checkbox` when**
- users need to choose between several options in forms
- a user must accept terms and conditions

**Avoid using `Checkbox` when**
- the user should choose only one option; use a [`Select`](/no/components/select/overview), [`Radio`](/no/components/radio/overview) or a single [`Switch`](/no/components/switch/overview) instead
- the user must choose from more than seven options; use a [`Select`](/no/components/select/overview) or [`Suggestion`](/no/components/suggestion/overview) instead
- something should be turned on or off; use a [`Switch`](/no/components/switch/overview) instead
