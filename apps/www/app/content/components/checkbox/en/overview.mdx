...
- the user needs to confirm something, for example that they have read and understood information

**Avoid checkbox when**
- the user should choose only one of several options; use a [radio](/en/components/docs/radio/overview) instead
- the user has many options to choose from; use a [select](/en/components/docs/select/overview) or [suggestion](/en/components/docs/suggestion/overview) instead
- the user should turn a feature or setting on or off; use a [switch](/en/components/docs/switch/overview) instead
...
Checkbox supports the `readonly` attribute on the `input` element to make the field non-editable and gives a visual indication that distinguishes it from editable fields. Although they cannot be changed, fields with the `readonly` attribute are still part of the tab order, and the information is included when the form is submitted.
...
Preferably place the response options vertically. This makes it easier to scan the list and read the options. For users who need to enlarge the text, horizontal lists can be challenging. If you need horizontal placement, use [chip with checkbox](/en/components/docs/chip/overview#chip-as-checkbox).
...
Read more about how to write [labels, questions and answer options in forms](/en/best-practices/content-work/questions).
