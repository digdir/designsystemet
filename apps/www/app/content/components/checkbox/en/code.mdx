...
### ReadOnly
From the web standard, checkbox is always `:read-only` because you cannot type text into it.

The design system ensures that the `readonly` attribute behaves as read-only by preventing `click` and `change`.

Use `input[type="checkbox"][readonly]` as a CSS selector if you need to extend the styling of a checkbox with the readonly attribute.
...
Use [useCheckboxGroup](/en/components/utilities/use-checkbox-group) to connect a parent checkbox that can select or clear all options in the group.
