...
For å lage en avatar stack i ren HTML brukes klassen `ds-avatar-stack` på en `<ul>`. Inne i denne plasseres `<li>` elementer rundt elmeneter med klassen `ds-avatar`. Bruk `<li>` med tekst for å legge til en indikator for skjulte avatarer.
...
**Merk:** I [nettlesere som ikke støtter `sibling-index()`](https://caniuse.com/wf-sibling-count) vil `data-expandable` ikke tegne riktig hvis den innheholder mer enn 12 avatarer.
