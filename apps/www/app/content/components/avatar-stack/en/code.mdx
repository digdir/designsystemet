...
You can also use an avatar with content `+xx`.
<Story story="AdditionalAvatars" layout='block' />

### Sizes
Set the avatar stack size using `data-size` or `--dsc-avatar-stack-size`. The size must be set on the avatar stack itself, rather than on the individual avatars, to ensure the overlap masking works correctly.
