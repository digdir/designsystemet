...
To create an avatar stack in plain HTML, use the `ds-avatar-stack` class on a `<ul>`. Inside this, place `<li>` elements wrapping elements with the `ds-avatar` class. Use a trailing `<li>` with text to add a indicator for hidden avatars.
...
**Note:** In [browsers that do not support `sibling-index()`](https://caniuse.com/wf-sibling-count), `data-expandable="fixed"` will not render correctly if it contains more than 12 avatars.
