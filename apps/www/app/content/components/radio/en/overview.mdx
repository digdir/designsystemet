...
There should not be more than seven options in one group. If users need more choices, consider using [`Suggestion`](/no/components/docs/suggestion) or [`Select`](/no/components/docs/select). If there is only one choice to make, a [`Switch`](/no/components/docs/switch) or a [`Checkbox`](/no/components/docs/checkbox) may be more suitable.
