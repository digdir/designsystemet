...
### Grouping
Use [`Fieldset`](/en/components/docs/fieldset) around a group of radio buttons to provide a common prompt (`<legend>`) and optionally a description (`description`).

<Story story="GroupEn" />

### With Error
Here we must use [`Fieldset`](/en/components/docs/fieldset), because it activates the correct style and ensures that the content has the correct connections for accessibility.
