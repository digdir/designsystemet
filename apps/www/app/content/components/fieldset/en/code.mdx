...
## Usage
Use `ds-fieldset` on the `<fieldset>` element, and place `<legend>` and `<ds-field>` elements inside.
...
When a [validation message](/en/components/docs/validation-message/code) with `data-field="validation"` is a child of `<fieldset>`, the containing `<ds-field>` elements will
automatically be updated to reflect the validation state.

<Story story="WithValidationEn" />
