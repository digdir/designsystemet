...
**1. En `Alert` som vises eller oppdateres etter at siden er lastet, må bli kunngjort automatisk av skjermlesere**

- Bruk `role="alert"` for kritiske meldinger som krever umiddelbar oppmerksomhet, og `role="status"` for mindre kritiske meldinger.

WCAG: [4.1.3](https://www.uutilsynet.no/wcag-standarden/413-statusbeskjeder-niva-aa/152)

**2. En `Alert` skal være plassert slik at den følger leserekkefølgen**
...
**3. `Alert`-teksten må forklare hva som har skjedd og hva det betyr for brukeren**

- Tekst som bare sier «Feil» eller «Advarsel» er ikke god nok.
