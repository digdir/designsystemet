...
**Use `Alert` when**
- the message is a short, informative and time-limited notification
- the information concerns an error that affects only one part of the system or a minor function
- the information relates to connectivity issues or API errors that are likely to resolve by reloading the page

**Avoid `Alert` when**
- individual form fields need validation, use the component's built-in error message instead [`ValidationMessage`](/en/components/docs/validation-message/overview)
- multiple error messages need to be summarised, use [`ErrorSummary`](/en/components/docs/error-summary/overview)
- the error prevents any further use of the service, use an error page instead
- the information is static and should always be visible, use [`Card`](/en/components/docs/card/overview) instead
