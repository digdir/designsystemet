...
## Retningslinjer
`Skeleton` er et alternativ til [`Spinner`](/no/components/spinner/overview) når du ønsker å indikere at innhold er i ferd med å lastes inn. `Skeleton` bidrar til at brukeren lettere forstår hva som kommer, og opplever ventetiden som kortere. Velg variant av `Skeleton` basert på typen innhold som lastes inn, for eksempel overskrifter, tekstblokker eller bilder. Forsøk å etterligne strukturen til det faktiske innholdet slik at overgangen til det ferdig lastede innholdet føles sømløs.
