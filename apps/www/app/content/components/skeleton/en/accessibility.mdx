...
When the user has `prefers-reduced-motion` set to `reduce`, the skeleton will use a 1.6-second animation. The animation is not decorative, so we do not disable it.
