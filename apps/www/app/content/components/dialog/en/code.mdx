...
Dialog depends on the attributes [`command` and `commandfor` (mozilla.org)](https://developer.mozilla.org/en-US/docs/Web/API/HTMLButtonElement/command) which are a relatively new part of the web-platform that allow [button](en/components/docs/button/) to perform actions on other elements declaratively. We automatically add [invokers-polyfill (npmjs.com)](https://www.npmjs.com/package/invokers-polyfill/v/0.5.2) to support older browsers.
