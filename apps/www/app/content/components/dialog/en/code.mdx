...
In HTML, you have to connect the `<dialog>` element to a trigger yourself through `command="show-modal"` or `command="--show-non-modal"` and `commandfor="DIALOG-ID"`.
...
<Card data-color="info">
  The attributes [`command` (developer.mozilla.org)](https://developer.mozilla.org/en-US/docs/Web/API/HTMLButtonElement/command) and `commandfor` are a relatively new part of the web-platform. They allow buttons to perform actions on other elements declaratively. We automatically add [invokers-polyfill (npmjs.com)](https://www.npmjs.com/package/invokers-polyfill/v/0.5.2) to support older browsers.
</Card>
...
### With command and without context

If you don't want to use `Dialog.TriggerContext`, you can use `command="show-modal"` or `command="--show-non-modal"` and `commandfor="DIALOG-ID"` to open the dialog from an external trigger.
