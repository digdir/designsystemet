...
Dersom du ikke vil bruke `Dialog.TriggerContext`, kan du bruke `command="show"` eller `command="show-modal"` og `commandfor="DIALOG-ID"` for å åpne dialogen fra en ekstern trigger.
...
I HTML må du selv koble `<dialog>`-elementet til en trigger via `command="show"` eller `command="show-modal"` og `commandfor="DIALOG-ID"`.
