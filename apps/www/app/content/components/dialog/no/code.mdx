...
Dialog er avhengig av attributtene [`command` og `commandfor` (mozilla.org)](https://developer.mozilla.org/en-US/docs/Web/API/HTMLButtonElement/command), som er en relativt ny del av webstandarden og lar [button](/no/components/docs/button/overview) utføre handlinger på andre elementer deklarativt. Vi legger automatisk til [invokers-polyfill (npmjs.com)](https://www.npmjs.com/package/invokers-polyfill) for å støtte eldre nettlesere ved behov.
