...
`Popover` is used to display more detailed or interactive supplementary information without taking the user out of context. It can be used as an extended solution when a [`Tooltip`](/no/components/docs/tooltip) is not sufficient. `Popover` is opened through an intentional user click and may contain text, links and simple form elements. The content should be short, relevant, and not critical for completing the task.
