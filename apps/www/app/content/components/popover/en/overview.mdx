...
**Use `Popover` when**
- you need to provide additional explanations that require more space than a short [`Tooltip`](/en/components/docs/tooltip), such as definitions or detailed instructions
...
**Avoid `Popover` when**
- you need to describe a symbol or keyboard shortcut; use [`Tooltip`](/en/components/docs/tooltip) instead
- the purpose is navigation or choosing between options; use [`Dropdown`](/en/components/docs/dropdown) instead
...
`Popover` is used to display more detailed or interactive supplementary information without taking the user out of context. It can be used as an extended solution when a [`Tooltip`](/en/components/docs/tooltip) is not sufficient. `Popover` is opened through an intentional user click and may contain text, links and simple form elements. The content should be short, relevant, and not critical for completing the task.
