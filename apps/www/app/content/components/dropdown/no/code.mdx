...
`Dropdown` bruker [`Popover`](/no/components/popover/code) internt.
...
Merk deg at elementet med `popover="manual"` bruker to klasses, `ds-popover` og `ds-dropdown`.
Du velger selv om du vil bruke `popover="manual"` eller bare `popover` for å åpne dropdownen som standard.
