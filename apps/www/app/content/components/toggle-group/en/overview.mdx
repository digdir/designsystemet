...
- the options represent answers in a form – use [`Radio`](/en/components/radio/overview) instead  
- the choice is an on/off setting – use [`Switch`](/en/components/switch/overview) instead  

## Example

### Icons only
A `ToggleGroup` without text should only be used when the icons are easy for users to understand. This may be suitable for expert users or internal services where the symbols are already familiar. Use [`Tooltip`](/en/components/tooltip/overview) to clarify the actions when using icons alone.
...
Write clear, consistent labels for each option, ensuring they follow the same linguistic pattern. Use [`Tooltip`](/en/components/tooltip/overview) to provide hints when needed, especially when using icons only.
