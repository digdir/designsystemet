...
**Unngå `Textfield` når**
- du trenger et helt enkelt inputfelt uten skjemalogikk, bruk heller [`Input`](/no/components/input/overview)
- brukeren skal velge fra et begrenset sett med alternativer, bruk heller [`Radio`](/no/components/radio/overview), [`Checkbox`](/no/components/checkbox/overview), [`Select`](/no/components/select/overview) eller [`Suggestion`](/no/components/suggestion/overview)  
