...
Field grupperer ledetekst, valgfri beskrivelse, skjemafelt (som input, textarea og select) og valideringsmelding.

Legg klassenavnet `ds-field` på `<ds-field>`-elementet. `<ds-field>` legger automatisk på tilgjengelighets-attributter, og støtte for valgfri telling av antall tegn.
`<ds-field>` kobler også valideringsmelding på tvers av felt, dersom den ligger inni et [fieldset](/no/components/docs/fieldset/code).

<Alert>
**Field eller fieldset?**

Field brukes for individuelle skjemafelt, mens [fieldset](/no/components/docs/fieldset/code) brukes for å gruppere flere felt sammen.
En god huskeregel er at **fieldset er et sett med field**.
</Alert>

**Merk**: React komponenten [textfield](/no/components/docs/textfield/code) har allerede field innebygd og skal derfor ikke settes inn i et field.
