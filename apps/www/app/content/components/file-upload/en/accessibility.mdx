...
File upload must always have a visible `Label` associated with it through `Field`, so that screen readers can communicate what should be uploaded. Use `Field.Description` to describe allowed file formats and sizes, so the user receives this information before uploading.