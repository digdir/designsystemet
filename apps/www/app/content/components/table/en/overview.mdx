...
## Text
Avoid long texts in table cells.
Keep content short and concise so it is easy to scan across rows and columns.
If you need to show more information, consider linking to a detail page.
Use headers to tell users what type of content they will find in the rows and columns.

As a general rule, content in cells should be left-aligned to make it easier to read and scan.
The exception is numbers and currency and sometimes form elements and buttons.
To ensure good readability on narrow screens, content in rows should generally be **top-aligned**.
