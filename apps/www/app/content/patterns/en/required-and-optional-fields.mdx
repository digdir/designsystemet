...
category: Completed
description: For users to understand which fields must be completed, required fields need to be marked clearly and consistently.
...
It is easiest for users when a page focuses on one clear question or action. A question or action can consist of one or more fields that naturally belong together. For example, when asking for an address, it is common to include several fields (street address, postcode, city), while still representing a single question or action.

In these cases, there is no need to mark each individual field as required, as long as it is clear from the label or help text that the user is expected to complete the information shown on the page.
