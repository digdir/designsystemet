---
title: Ask the user for date and time
sidebar_title: Date and time
category: Ask the user about...
...
How you ask users for date and time should be based on the information you actually need. Some dates are easy to remember, such as a date of birth, while others must be read from documents or cards. In some situations, an approximate date is sufficient, such as “December 2017”, or a date relative to today, such as “4 days from now”. In other cases, users need to choose a date or time from a predefined set of options.

Avoid making the task more complicated than necessary. Custom-built date pickers often result in lower accessibility and more friction than simple text inputs. They can be difficult to use with a keyboard, provide poor screen reader support, and require unnecessary navigation when the user already knows the date.

## Known dates

When asking for known dates, such as date of birth or wedding date, simple text inputs work well. Users already know the answer and often prefer to type it directly instead of navigating a calendar.
...
In some cases, users need to choose a specific date close to today, for example when booking an appointment or setting a deadline. In these situations, visual support can be helpful. An [`Input`](/en/components/docs/input/overview) with `type="date"` allows users to either type the date directly or use the browser’s built-in date picker. This often provides a good balance between flexibility and support.
...
Place the fields in a logical order and validate that the end date is not earlier than the start date. If the dates depend on each other, error messages should clearly explain what is wrong and how the user can fix it.

<Story story="StartEndDateEN" />

## Time – start and end

When users need to provide a time interval, such as opening hours or the duration of a meeting, an [`Input`](/en/components/docs/input/overview) with `type="time"` works well. It allows users to type the time directly while also benefiting from browser support.
...
For example, you can allow users to provide an approximate date in a free text field, or offer separate fields for month and year only, as shown in the example below.
...
If you know that users need to choose a date or time from a limited set of options, for example available appointment slots in the coming week, it may be more efficient to present these as ready-made choices.
...
Note: The example below is not connected to an actual data source. It illustrates how users can narrow down the options before seeing the available appointment slots.

<Story story="PredefinedOptions2EN" />

You should only show options that have available appointments, and avoid disabling weeks without availability, as this may create confusion. Explain in advance that only weeks with available slots are shown, so users understand why some weeks are not available.

## Summary

Start with the user’s need and choose the solution that allows them to complete the task as quickly as possible. Standard input fields are often more accessible and faster to use than advanced date pickers.

- Show predefined options when the alternatives are limited  
