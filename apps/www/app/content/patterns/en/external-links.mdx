...
#### Reverse tabnabbing
When you open a link in a new tab using `target="_blank"`, the site you’re linking to can gain access to the original window—and in the worst case, replace it with something malicious. This is called reverse tabnabbing.

Modern browsers usually prevent this automatically. Still, it's a good idea to add `rel="noopener"` as an extra layer of protection, especially for users with older browsers. This prevents the new tab from gaining control over the original one.
