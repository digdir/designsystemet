---
title: Obligatoriske og valgfrie skjemafelt
sidebar_title: Obligatoriske felt
category: Ferdig
description: For at brukerne skal forstå hvilke felter som må fylles ut, må obligatoriske felt merkes tydelig og konsekvent.
partners: Digdir, Nav, Skatteetaten ++
date: 2023-11-28
order: 10
---

Det er flere måter å markere obligatoriske felt på som [oppfyller kravene til merking](https://www.uutilsynet.no/veiledning/skjema/38#ledetekster_og_instruksjoner). Klarer vi å gjøre dette likt på tvers blir det lettere for innbygger å forstå og kjenne igjen mønsteret på tvers av løsningene våre. Det vil alltid være unntak og ulike kontekster som krever ulik merking.

En generell retningslinje er at vi bør unngå å be om informasjon vi ikke trenger, altså unngå valgfrie felt.

Her tar vi for oss 3 eksempler.

1. [Når du har ett spørsmål per side](#1-ett-spørsmål-per-side)
2. [Når du har flere spørsmål per side](#2-når-vi-må-ha-flere-spørsmål-per-side)
3. [Når du har en kombinasjon av obligatoriske og valgfrie felt](#3-når-vi-må-ha-en-kombinasjon-av-obligatoriske-og-valgfrie-felt)

## 1. Når du har ett spørsmål per side
Det er enklest for brukeren når én side handler om ett tydelig spørsmål eller én handling. Et spørsmål eller en handling kan bestå av ett eller flere felter som naturlig hører sammen. Ber du for eksempel brukeren om adresse, vil det ofte være flere felter (gateadresse, postnummer, poststed), men fortsatt ett samlet spørsmål eller én handling.

I slike tilfeller trenger vi ikke å markere hvert enkelt felt som obligatorisk, så lenge det er tydelig i ledeteksten eller hjelpeteksten at brukeren forventes å svare på det som vises på siden.


<Image
  src='/img/obligatorisk-eks1.png'
  alt='Skjermbilde av skjema med ett spørsmål per side, der brukeren får ledeteksten "Oppgi fødselsnummeret til personen du skal gifte deg med" og en hjelpetekst som sier "Har personen D-nummer bruker du det." Videre følger et inputfelt for fødsselsnummer.'
  boxShadow={false}
/>

Dersom det er frivillig å fylle ut en av sidene, kan dette også komme frem i ledeteksten eller hjelpeteksten.

<Image
  src='/img/obligatorisk-eks1b.png'
  alt='Skjermbilde av skjema med ett spørsmål per side, der brukeren får spørsmålet "Vil du oppgi kontaktopplysninger som kan brukes av andre?" De får i tillegg en hjelpetekst som sier "Det er frivillig å oppgi e-postadresse, nettside og telefonnummer. Være oppmerksom på at kontaktopplysningene er tilgjengelig for alle og kan bli brukt til markedsføring." Det er til slutt en checkbox der du kan huke av for "Jeg vil oppgi kontaktopplysninger"'
  boxShadow={false}
/>

## 2. Når du har flere spørsmål per side

Hvis vi _må_ ha flere spørsmål på en side, kan vi informere tydelig i toppen om at _alle felt må fylles ut_. Vi skal bare spørre brukeren om informasjon vi absolutt trenger, og unngå valgfrie felt. (Kan du ikke unngå valgfrie felt, se [eksempel 3](#3-når-vi-må-ha-en-kombinasjon-av-obligatoriske-og-valgfrie-felt)).

- Bruk for eksempel en gul tag, som vist i eksemplet under.
- Bruk teksten **«Alle felt må fylles ut»**. \
  (_Ikke_ bruk _"Obligatorisk"_ eller _"Påkrevd"_)
- Pass på at det ikke er for mange spørsmål på samme side. Det som ligger på samme side bør ha samme tema eller spørsmål som er relatert til hverandre.
  <Image
    src='/img/obligatorisk-eks2.png'
    alt='Skjermbilde av skjema med flere spørsmål per side.'
    boxShadow={false}
  />

## 3. Når du har en kombinasjon av obligatoriske og valgfrie felt

En kombinasjon av obligatoriske og valgfrie felt er ikke ideellt! Men det vil være situasjoner der vi ikke kommer unna det. Hvis vi _må_ ha med valgfrie felt i tillegg til de obligatoriske, bør vi markere alle feltene hver for seg. Da trenger vi ikke å informere i toppen.

- Informer tydelig med taggen «Må fylles ut» eller «Valgfritt» bak hvert spørsmål/hver ledetekst.
- Ikke bruk ordene «Obligatorisk» eller «Påkrevd».
- Bruk for eksempel gul tag for «Må fylles ut» og blå for valgfritt.
- Ikke bruk stjerne for å markere felt som må fylles ut.

<Image
  src='/img/obligatorisk-eks3.png'
  alt='Skjermbilde av skjema med flere spørsmål per side, både obligatoriske og valgfrie.'
  boxShadow={false}
/>

<br />

---

## Kilder og relevant informasjon:<br/>

- [UU-tilsynet](https://www.uutilsynet.no/veiledning/skjema/38#ledetekster_og_instruksjoner)<br/>
- [Gov UK - Question pages](https://design-system.service.gov.uk/patterns/question-pages/)
- [NN Group - Marking Required Fields in Forms](https://www.nngroup.com/articles/required-fields/)

<br />

<Contributors
  headingLevel={2}
  authors={[
    'Digdir',
    'Nav',
    'Skatteetaten',
    'Brønnøysundregistrene',
    'Oslo kommune',
  ]}
/>

