...
Color names in the Design System are divided into 3 parts: Name of the color scale, group (area of use), and variant.
These parts describe how the colors are built up and how they should be used.

<Image
  src='/img/color-semantic.png'
  alt='Showing the structure of a semantic color'
  boxShadow={false}
...
These groups describe the areas of use for the colors. The Text group, for example, should be used for text and icons.

<Image
  src='/img/fargeskala-success.png'
  alt='Shows how the color scales are divided into groups'
  boxShadow={false}
...
Tinted, for example, means that the color has a hint of color in it, while Hover means that the color is intended for an interactive state.

## Overview and Explanation of Colors
Each color scale consists of a total of 16 colors, designed to cover various needs in the design. 
...
### Surface
Surface colors are used to color elements that lie above the background colors, such as panels or cards.
These colors function as foreground colors and help create depth in the design by separating elements from the background.
In dark mode, these four colors become gradually lighter, with Surface-active as the lightest.

* `Surface-default` is completely white in light mode and is used as the standard background color for elements.
* `Surface-tinted` gets a hint of color in it and can be used to separate elements from the background.
...
Common use areas are dividers and decorative frames.
The color should not be the only visual indicator that an element is interactive.
* `Border-default` is used on form components or other meaningful frames. 
...
It should be used for the main content and primary text on a page.
...
Base-hover and Base-active colors are generated based on the lightness or darkness of the Base-default color to create smooth visual transitions between states. 
...
* `Base-default` can be used to color solid backgrounds for elements. The color (hex code) that is chosen in the theme builder is placed under Base-default.
...
<Image
  ...
  caption='Shows boxes with Base-default as the background and Base-contrast-default as the text color.'
/>
