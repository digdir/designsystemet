---
title: Composition
description: Information and usage of asChild to solve composition.
date: 2024-01-31
...
This happens using Radix's `Slot` component. <sup>1</sup>

`Slot` merges its props down to the component that exists as a child element. In the case above, the `Button`'s props are added to the `Link` component, and an `a` tag is rendered.
...
`Slot` solves this by adding all props to the child element of the component, thereby providing type safety.
...
When `Slot` merges props, it will create a new function that calls all event handlers defined on `Button` and `Link`. The function that is on `Link` will be called first.
...
Several components in the Design System support `asChild` with standard elements. If you change this, you need to ensure that accessibility is maintained. It's rare that you need to change the underlying element, but it's more realistic that you will want to use your own component.
