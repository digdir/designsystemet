---
title: Sizes
description: Information about how to use sizes in frontend.
date: 2025-01-10
...
HTML data attributes are used to define the size of a component. The components can use all available sizes.

Our `size` variables (`--ds-size-*`) change based on the local font size (formula based on `em`). You can easily change it by using `data-size` for our recommended sizes, or define your own sizes by setting `font-size` in CSS.

## `data-size`

This data attribute is used to change sizes on all components under it.
