...
Our `size` variables (`--ds-size-*`) change based on the local font size (formula based on `em`). You can easily change this by using `data-size` for our recommended sizes, or define your own sizes by setting `font-size` in CSS.
