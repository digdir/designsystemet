...
This article assumes that you have already defined the colors you will use with the help of the [Theme Builder](/en/fundamentals/start-here/own-theme).
...
`color` and `background-color` are redeclared to default values on the element with `data-color-scheme`. This is necessary as the variables are not inherited past the point where they are redefined.
...
You can override this by setting custom values for `color` and `background-color` on the element.
