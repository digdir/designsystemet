...
- Use `data-color-scheme="light | dark | auto"` to set dark/light mode.
  - corresponds with `Appearance › Color schemme` in Figma
...
We aim for the components to function well across various test setups. Designsystemet is tested in JSDOM, HappyDOM, and Playwright using Safari, Firefox, Chrome, and Edge. However, there will always be frameworks, test environments, and meta-frameworks that handle asynchronous rendering or specific edge cases in different ways.
