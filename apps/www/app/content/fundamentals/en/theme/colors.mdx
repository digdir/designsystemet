...
## Contrast
The colour system is structured to meet contrast requirements. The Theme Builder ensures that contrast requirements are met through the colours generated from the brand colour. As a result, colours intended for text will always have sufficient contrast against background colours.

Examples:

- `Text-default` always has sufficient contrast against all `background` and `surface` colours.
- `Text-subtle` always has sufficient contrast against all `background` colours and `surface-default`.
- This applies regardless of which base colour you have chosen.

The `Base-default` colour will always be the same as the colour you selected. This is to preserve your brand as accurately as possible. You are therefore responsible for ensuring that the colour you choose meets contrast requirements for its intended use. The [Designsystemet theme generator](https://theme.designsystemet.no) will notify you of any contrast issues.
