...
Designers can easily switch between different modes in Figma and see information about the selected mode. Those without write access will not be able to see which mode is selected or its information, such as `Color scheme`, `Theme`, or `Typography`. This information is only available to those with editing access or those using Dev Mode in Figma.
...
If a mode is set on an entire frame, it is only visible in Dev Mode when the frame itself is selected. If you instead click on an individual component inside the frame, the information about the selected mode will not be visible.
...
In the theme builder, you can define colors and other visual settings for your design. See the guide [Use the Design System with your own theme](/en/fundamentals/for-designers/custom-theme) to get started.
