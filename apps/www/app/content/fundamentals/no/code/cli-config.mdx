---
title: CLI Config
description: Informasjon om korleis du bruker CLI config fil.
date: 2025-10-06
category: Kode
color: blue
icon: CodeIcon
published: true
order: 60
---

Å bruke config fil med CLI gjer at du kan ha meir kontroll over fargane dine, lettare oppdatere og vedlikehalde dei.

## Kom i gang

Config fila kan heite det du vil, men bruker du `designsystemet.config.json` vil me finne den automatisk om du køyrer kommando i samme mappe. 
Du kan legge den kor du vil i prosjektet ditt. 

Har du plassert fila di ein anna plass, eller med eit anna navn, kan du bruke `--config <path>` i CLI-kommandoar.

```json
{
  "$schema": "node_modules/@digdir/designsystemet/dist/config.schema.json",
  "themes": {
    "my-theme": {
      "colors": {
        "main": { "primary": "#0062BA" },
        "neutral": "#1E2B3C",
        "support": { "extra1": "#F45F63" }
      },
      "borderRadius": 4,
      "typography": {
        "fontFamily": "Inter"
      }
    }
  }
}
```

Over har me definert eit tema som heiter `my-theme` med nokre fargar og border radius.
Legg merke til at `neutral` er definert som ein streng, medan `main` og `support` er objekt. 
Dette er fordi `neutral` berre har ein farge, medan `main` og `support` har fleire.

## Struktur

| Navn | Type | Påkrevd | Forklaring |
| ---- | ---- | ------- | ----------- |
| themes | Objekt | Ja | Inneheld alle tema du vil definere. Ny nøkkel er namnet på temaet. |
| outDir | Streng | Nei | Mappa der design tokens skal lagrast. Standard er `./design-tokens`. |
| clean | Boolean | Nei | Slett utdata-mappa før du lagar tokens. Nyttig for å fjerne utdaterte filer. |

### Themes

| Navn | Type | Påkrevd | Forklaring |
| ---- | ---- | ------- | ----------- |
| colors | Objekt | Ja | Inneheld alle fargar for temaet. |
| borderRadius | Tal | Ja | Base border radius for temaet. |
| typography | Objekt | Nei | Inneheld typografi innstillingar for temaet. |
| overrides | Objekt | Nei | Inneheld overskrivingar for fargar og severity-fargar. |

### Colors
| Navn | Type | Påkrevd | Forklaring |
| ---- | ---- | ------- | ----------- |
| main | Objekt | Ja | Hovudfargar for temaet. |
| neutral | Streng | Ja | Nøytral farge for temaet. |
| support | Objekt | Nei | Støttefargar for temaet. |

Objektet for main og support inneheld fargar, der navnet er fargen sitt namn og verdien er ein hexkode.

```json
{
  "main": {
    "primary": "#0062BA",
    "accent": "#1E98F5"
  },
  "neutral": "#1E2B3C"
}
```

### Overrides
| Navn | Type | Påkrevd | Forklaring |
| ---- | ---- | ------- | ----------- |
| colors | Objekt | Nei | Inneheld overskrivingar for individuelle fargesteg |
| severityColors | Objekt | Nei | Inneheld overskrivingar for severity-fargar |

#### overrides.colors
| Navn | Type | Påkrevd | Forklaring |
| ---- | ---- | ------- | ----------- |
| "fargeNavn" | Objekt | Nei | Overskrivingar for dominant fargar |

Kvart objekt begynner med ein nøkkel som er namnet på fargen du vil overskrive.
I dette objektet legg du så til fargesteg som objekt, der kvart fargesteg har ein `light` og ein `dark` verdi som at i mot ein hexkode.

```json
{
  "overrides": {
    "colors": {
      "dominant": {
        "background-default": {
          "light": "#ff0000",
          "dark": "#000fff"
        },
      }
    }
  }
}
```

## Komplett Eksempel

```json
{
  "$schema": "node_modules/@digdir/designsystemet/dist/config.schema.json",
  "outDir": "./design-tokens",
  "themes": {
    "my-theme": {
      "colors": {
        "main": { "primary": "#0062BA", "accent": "#1E98F5" },
        "neutral": "#1E2B3C",
        "support": { "extra1": "#F45F63", "extra2": "#E5AA20" }
      },
      "borderRadius": 4,
      "typography": {
        "fontFamily": "Inter"
      },
      "overrides": {
        "colors": {
          "primary": {
            "background-default": {
              "light": "#ff0000",
              "dark": "#000fff"
            },
            "text-default": {
              "light": "#00ff00",
              "dark": "#00ffff"
            }
          }
        },
        "severityColors": {
          "success":  "#28a745",
          "warning": "#ffc107",
          "danger": "#dc3545",
          "info": "#17a2b8"
        }
      }
    }
  }
}
```
