...
published: true
order: 60
search_terms: farger, colors, theming
---

Denne artikkelen føreset at du allereie har definert fargane du skal bruke ved hjelp av [Temabyggeren](/no/fundamentals/start-here/own-theme).
...
`color` og `background-color` blir redeklarert til standard verdiar på elementet med `data-color-scheme`. Dette er nødvendig fordi variablane ikkje blir arva forbi det punktet dei blir redefinert. 
...
Du kan overstyra dette med å setje eigne verdiar for `color` og `background-color` på elementet.
