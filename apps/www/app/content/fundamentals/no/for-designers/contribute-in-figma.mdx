...
- Størrelser håndteres med [Modes i Figma](http://localhost:5173/no/fundamentals/for-designers/modes-in-figma). Du trenger ikke lenger å lage egne varianter for `Small`, `Medium` og `Large`. Så lenge du kobler komponenten til tokens for typografi, avstander og størrelser, vil den tilpasse seg riktig størrelse basert på valgt mode. Dette gjør det både enklere å vedlikeholde komponentene og enklere å bruke dem i ulike størrelser. 
