...
Feil som hindrer brukeren i å komme videre kan skyldes både systemfeil (link til egen artikkel) og brukerutløste feil. Denne artikkelen tar for seg brukerutløste feilmeldinger i selvbetjeningsløsninger og skjema. Hvordan kan vi unngå dem, og hvordan kan vi best mulig hjelpe brukerne å komme seg videre når en feil likevel har oppstått?

## Vi skiller mellom feilmeldinger, myke valideringer og systemvarsler

Vi bruker feilmeldinger når brukerne har gjort en feil som hindrer dem i å komme videre. Da skal vi beskrive problemet i en feilmelding og forklare hva brukerne må gjøre for å komme videre. Feilmeldingen skal være lett å se og klart og presist formulert.

Myke valideringer bruker vi til å gi brukeren informasjon, uten å stoppe dem fra å sende inn eller gå videre til neste steg.

[Accordion - Mer om myke valideringer ]
Myke valideringer er meldinger som ikke stopper brukeren fra å sende inn eller gå videre til neste steg i prosessen, men som bare gir brukeren informasjon. Slike meldinger kan for eksempel presenteres i en [`alert`](/docs/komponenter-alert--docs) av typen "info" eller "success".

Systemvarsel bruker vi når vi vil fortelle brukerne om noe viktig de må få med seg.
...
Generelt anbefaler vi at hvis brukerne prøver å gå videre uten å ha fylt ut alt, eller de har gjort feil, oppsummerer like over "Gå videre"/"Send inn"-knappen. Vi vil vise oppsummeringen i nærheten av Neste-knappen for at brukerne skal forstå sammenhengen mellom feilen og hvorfor de blir hindret i å gå videre.
...
Det er løsningen eller tjenesten du lager, som bestemmer om du bør plassere oppsummeringen øverst på siden eller lenger nede, før knappen som tar brukerne videre. Det viktigste er at brukeren lett ser oppsummeringen.
...
- Vi viser feilmeldingen under feltet med feil
- Når det er flere feil, oppsummerer vi dem nederst på siden.

Plasser feilmeldinger som tilhører et bestemt felt på siden så nært feilen som mulig.

## Utseendet på feilmeldinger

Vi viser feilmeldinger i rød tekst og med trekant-symbolet.
...
Vi ønsker å utheve felter som har valideringsfeil. For å få dette til bruker vi en kombinasjon av virkemidler.

**“Fysiske” endringer**
Øk tykkelse på rammen rundt feltet. Dette vil utheve uavhengig av fargevalg.

**Fargevalg**
Felter med feilmeldinger farges røde. Dette er et godt etablert . Husk at feltet må ha 3:1 kontrast til bakgrunnsfargen. Les mer hos Tilsynet Om du også bruker rød tekst til feilmeldingen må den minst være 4.5:1.

### Kontraster og symbolbruk

Vi bruker en rød varseltrekant med utropstegn i, i tillegg til selve feilmeldingsteksten i rødt. Vi anbefaler å ha med et symbol fordi det kan være fint med en visuell indikator i tillegg til tekst.
