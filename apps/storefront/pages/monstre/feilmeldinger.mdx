...
Det beste er om vi klarer å forhindre at feil oppstår. For å unngå at feil oppstår kan vi i forkant være tydelig på forventninger og krav, gi eksempler på korrekt utfylling, og være fleksible på formattering. Vi må la brukere kunne ombestemme seg og alltid gi dem en vei ut.
...
- [Detaljerte feilmeldinger](#detaljerte-feilmeldinger) som tilhører et bestemt felt i et skjema og validerer brukerens inndata.
- [Oppsummerende feilmeldinger](#oppsummerende-feilmeldinger) som kan vise en eller flere feilmeldinger i toppen av en side eller like over "Gå videre"/"Send inn"-knappen.
- For andre type feil som ikke tilhører et skjemaelement og ikke validerer brukerens inndata, kan [alert](/docs/felles-alert--docs)-komponenten brukes. For eksempel for systemfeil, eller til myke valideringer. Myke valideringer er valideringsmeldinger som ikke stopper bruker fra å sende inn eller gå videre til neste steg i prosessen, men som benyttes til å gi brukeren ulike former for informasjon.
...
- Når bruker trykker "Gå videre" eller "Send inn"
...
Feilmeldinger bør som regel _ikke_ vises mens bruker fyller ut feltet. Dette er fordi mange bruker tastatur til å navigere seg gjennom skjema og vil ikke nødvendigvis starte på toppen og jobbe seg nedover. Det vil da være forstyrrende om det dukker opp feilmeldinger på felter man ikke har jobbet med enda. Om du har feilmeldinger som skal vises idet bruker forlater feltet er det viktig å sjekke om brukeren faktisk har skrevet noe i feltet før du viser feilmeldingen.

Påkrevde felter bør aldri aktivere feilmelding før bruker prøver å gå til neste side eller forsøker å sende inn skjema. Om de aktiveres når brukeren forlater feltet kan det bli et irritasjonsmoment. Vær heller tydelig på at feltene er obligatoriske. Se hvordan du markerer [obligatoriske og valgfrie skjemafelt](/monstre/obligatoriske-og-valgfrie-felt).
