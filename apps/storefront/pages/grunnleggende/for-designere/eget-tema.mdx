...
2. Gå til [theme.designsystemet.no](https://theme.designsystemet.no). Legg inn dine farger og velg `Kopier` > `Light theme`. [Les mer om fargesystemet her](/grunnleggende/for-designere/fargesystem).
...
2. Installer designystemets [plugin for Figma](https://github.com/digdir/designsystemet/tree/next/plugins/figma-sync).
3. Gå til [theme.designsystemet.no](https://theme.designsystemet.no) og generer fargeskalaen ut i fra dine profilfarger.
4. Kopier JSON-koden og lim den inn i pluginen du installerte i steg 3.
