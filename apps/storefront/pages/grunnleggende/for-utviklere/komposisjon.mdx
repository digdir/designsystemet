...
`Slot` merger sine props ned på den komponenten som ligger som underordnet element. I tilfellet over så vil `Button` sine props bli lagt til på `Link`-komponenten, og
det er ein `a`-tagg som blir rendra ut.
Når du bruker `asChild`, så kan du ikkje ha meir enn eit underordna element, men du kan ha så mange du vil inne i det elementet.
...
`Slot` fikser dette ved at du legger alle props på det underordna elementet av komponenten, og du får dermed typesafety.
...
Dersom ein av event handlerane er avhengig av `event.defaultPrevented`, så må du passe på at rekkefølga er korrekt. <sup>2</sup>
...
Fleire Designsystemet komponentar støttar `asChild`, med standard element. Endrast dette må du passe på at tilgjengelegheit er ivaretatt. Det er sjeldent at du behøver å endre det underliggande elementet, men det er meir realistisk at du vil bruke din eigen komponent.

Ønsker du å bruke ein eigen komponent må du passe på å spre alle props, og ha støtte for `ref`. <sup>3</sup>
