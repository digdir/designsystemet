...
Det kan vere utfordrande å skape harmoniske fargeskalaer programmatisk – skalaer som både ser gode ut for det menneskelege auget og fungerer godt i eit system med omsyn til kontrast og tilgjengelegheit. 
...
Det me såg føre oss var å lage eit verktøy der brukaren kunne velje seg ein farge, og få generert ein fargeskala basert på denne i lysare og mørkare valørar. 

## Behagelige fargeskalaer for mennesker
Det fins veldig mange ulike måtar å lage fargar på. 
Det er eit hav av fargerom og teknikkar tilgjengelig som løyser mange ulike behov. 
For Designsystemet sin del, så var me ute etter å lage fargar som såg fine og balanserte ut for det menneskelege auget.
...
I bildet over så blir ein raud-farge treft av eit lys med ulik intensitet. 
...
Dette var utgangspunktet for måten me ønskte å lage fargeskalaene på, ved å emulere korleis lyset påverkar fargar i den verkelege verda.


## Ulike teknikkar for å lage fargeskalaer
To av dei vanlegaste måtane å lage fargar på er ved å bruke RGB- og HSL-fargeromma.
Me skal no sjå nærare på korleis desse metodane fungerer og korleis resultata ser ut. I eksempla brukar me den raude profilfargen til Digdir som utgangspunkt for å lage skalaene. 
...
I bildet over så har me teke den raude profilfargen og lagt til 20 % kvitt og 20 % svart i kanalane bortover for å lage ulike valørar. 
Dette synst me laga veldig fine valørar fordi det emulerte litt korleis lyser påverkar fargar i den verkelege verda, og korleis me mennesker opplever fargar.
...
* Hue (fargetone) beskriv sjølve fargen og angir på en skala frå 0° til 360°, som for eksempel raud, grøn eller blå. 
* Saturation (metning) angir kor intens eller grålig fargen er på ein skala frå 0 % til 100 %. 
...
I biletet over har me tatt utgangspunkt i den raude profilfargen til Digdir (`67% lightness`) og sett faste lightness-verdiar for å lage ulike valørar.
Samanlikna med RGB-teknikken, så ser me at fargane blei mykje meir saturerte oppover i skalaen. 
Dette gjer meining fordi me har ikkje justert på satueringsverdiane. 
Dette illustrerar veldig godt ei utfordring med mange av fargeromma som brukar lightness for å definere lysstyrka i fargen.
Endrar me kun på denne verdien så endar fargane ofte opp med å sjå oversaturerte og ubalanserte ut.
...
Eit fargerom som me var veldig fornøgde med, var OKLCH-fargerommet. 
...
Men korleis kunne me sy dette saman i eit system som genererer fargeskalaer basert på kontrast og tilgjengelegheit?

## WCAG og offentlege nettsider 
Vår tilnærming til utviklinga av fargesystemet er å sikre at alt me gjer oppfyller alle AA-krava i WCAG.
...
om fargane ikkje har kontrast i det heile tatt, er poengsummen `1:1`. 
...
Denne verdien blir rekna ut basert på dei raude, grøne og blå komponentane i fargen, der grøn bidreg mest til den opplevde lysstyrka. 
...
I biletet  over er det 2 farga bakgrunnar med svart tekst i seg. 
...
HSLuv justerer fargane så dei held seg balanserte, også i RGB-området, medan OKLCH brukar ein nyare modell som gir meir presise og jamne fargar. 
...
I biletet over vises to fargeskalaer i OKLCH-fargerommet som begge baserer seg på profilfargane til Digdir. 
...
Dette betyr i praksis at ein kan generere opp så mange fargeskalaer som ein vil, og sikre at kontrastane mellom dei ulike stega i fargeskalaene alltid vil være like.
...
Det blir ofte brukt når ein har eit datasett og ønskjer å finne verdiar som ligg mellom punkta. 
...
For vår del handla det om å trekkje ei linje gjennom RGB-fargerommet.
...
For å sikre at kontrasten mellom to fargar alltid blir like, så må den relative luminansen også være lik.
...
I biletet over er det 2 fargar heilt til venstre med luminansverdiane `0.12` og `0.33`. 
...
For å gjere dette ser me først på kva luminansverdiar me har å jobbe med i fargane heilt til venstre.
Begge desse verdiane er lågare enn `0.7`. Me må derfor 'tinte' fargane for å få dei til å nærme seg `0.7`.

I eksempelet over tintar me fargane med 10% om og om igjen, heilt til me oppnår luminansverdien som er så nær `0.7` som mogleg.
I iterasjon 3 av blåfargen (fargen rett til ventre for resultatet) ser me at luminansverdien er `0.34`. 
Dette er fortsatt ikkje i nærleiken av `0.7`, så me må derfor fortsetje med å "tinte" fargane.
Etter 15 iterasjonar for blåfargen, og 11 iterasjonar for grønfargen, har me funne den næraste luminansverdien til `0.7` som me kan med denne teknikken.

Målet var å finne luminansverdien `0.7` for fargane, men som biletet over viser så blei resultatet `0.72` og `0.71`.
Dette viser greitt korleis interpoleringen fungerer, men det er ikkje nøyaktig nok for vårt formål.
I koden vert det nytta ein meir effektiv metode, kalla binært søk, for å navigere seg gjennom fargerommet. 
...
I biletet over ser du 4 fargeskalaer som blir generert basert på dei 4 fargane som du finn i toppen av biletet.


### "Base"-gruppa
Denne gruppa fungerar litt annleis enn dei statiske gruppene. 
...
I biletet over ser du ein sekjon til venstre der `Base Hover` og `Base Active` fargane blir lysare, og ein seksjon til høgre der fargane blir mørkare.
Måten me lagar desse fargane på er at me legger til, eller trekker frå, lightness frå fargen i HSLuv fargerommet.
Grunnen til at me brukar HSLuv her er fordi me vil at fargane skal auke lysstyrke med ein fast verdi som opplevast som naturleg for menneskeauget.
...
Sidan me genererer fargar basert på kontrast, er det ikkje alle fargetonar som oppfører seg likt for det menneskelege auget.
Fargen grøn er veldig dominerande i lys modus, medan blåfargen er svært framståande i mørk modus.
Me vil derfor jobbe med å jevne ut satureringa for alle fargetonane slik at dei opplevast som meir balanserte.

Me vil og fortette å jobbe med å få mørk modus til å fungere betre i systemet.
...
så kan du ta kontakt med oss på kommunikasjonskanalane våre som du finn i footeren på nettsida.
