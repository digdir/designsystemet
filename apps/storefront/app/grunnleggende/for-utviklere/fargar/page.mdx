...
`Button` er ein komponent som ikkje arvar farge.
...
Er komponentar som gjer meir av fargen sin, og `data-color` må bli direkte satt. Dette er komponentar som `Alert` og `ErrorMessage`.
Å "gjere meir av fargen sin" betyr at det t.d. kan være ikon som blir satt basert på farge.
