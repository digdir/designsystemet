...
async function checkEmbedderStatus() {
  try {
    ...
    console.log('  Filterable attributes:', settings.filterableAttributes || 'None configured');
    console.log('  Searchable attributes:', settings.searchableAttributes || 'All attributes');
    console.log('');
    
    // Check what embedder UID the server expects
    const expectedEmbedderUID = env.MEILISEARCH_EMBEDDER_UID || 'azure-openai-small';
    ...
    try {
      ...
      console.log('❌ Hybrid search with embedder failed:', embedderError.message);
