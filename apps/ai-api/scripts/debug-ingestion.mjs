  ...
  try {
    ...
    if (fs.existsSync(testFilePath)) {
      ...
      const {
        ...
        lang,
      } = extractTextFromFile(testFilePath, content);
      ...
      console.log(
        `  Content preview: ${extractedContent.substring(0, 100)}...`,
      );
      ...
      const document = {
        id: crypto
          .createHash('md5')
          .update(testFilePath + chunks[0] + 0)
          .digest('hex'),
    ...
    if (!fs.existsSync(testFilePath)) {
      ...
      console.log(
        `📂 Found component directories: ${componentDirs.slice(0, 3).join(', ')}`,
      );
      ...
      if (componentDirs.length > 0) {
        ...
        if (mdxFile) {
          ...
          const {
            ...
            content: extractedContent,
            lang,
          } = extractTextFromFile(mdxPath, content);
