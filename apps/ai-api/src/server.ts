...
// Create Meilisearch client for searching (uses search key)
...
// Create Meilisearch client for admin operations (uses admin key)
...
async function generateChatCompletion(messages: Array<{role: string, content: string}>) {
...
function formatChatResponse(content: string, sources: Array<{title: string, url: string}> = []) {
...
async function vectorSearch(query: string, limit = 5) {
  try {
    ...
    try {
      ...
      console.warn('Embedding generation failed, falling back to keyword search:', embeddingError);
    }
    ...
    if (embedding) {
      try {
        ...
        const embedderUid = process.env.MEILISEARCH_EMBEDDER_UID || 'azure-openai-small';
        ...
        try {
          ...
          console.log('Vector search with embedder successful');
        } catch (hybridError) {
          // If embedder-based search fails, try just vector search
          console.warn('Hybrid search with embedder failed, trying just vector:', hybridError);
          searchResponse = await index.search(query, {
            limit: limit,
            vector: embedding
          });
        ...
        console.warn('All vector search attempts failed, falling back to basic search:', vectorSearchError);
        // Fall back to basic search
        searchResponse = await index.search(query, { limit });
      }
    } else {
      // If we couldn't generate embeddings, just do basic search
      searchResponse = await index.search(query, { limit });
    }
...
app.post('/api/ai-search', async (req, res) => {
  ...
  try {
    ...
    const formattedResponse = formatChatResponse(response, sources as Array<{title: string, url: string}>);
