...
const MIGRATION_GUIDE_URL =
  'https://designsystemet.no/en/blog/migrate-simpler-color';
...
export function TokenExport({
      ...
      switch (message.type) {
        case 'import-success':
          ...
          dispatch({
            ...
            notification: { kind: 'success', text: 'Exported to Figma.' },
          });
        ...
        case 'import-error':
          ...
          dispatch({
            ...
            notification: {
              kind: 'error',
              text: (message.message as string) || 'Export failed.',
            },
          });
        ...
        case 'prepare-variables-result': {
          ...
          const result =
            (message.result as { status?: string; message?: string }) || {};
          if (result.status === 'error') {
            dispatch({
              type: 'set-notification',
              notification: {
                kind: 'error',
                text: result.message || 'Could not prepare the variables.',
              },
            });
          } else {
            dispatch({
        ...
        notification: {
          ...
          text:
            error instanceof Error
              ? error.message
              : 'Could not generate tokens from config.',
        },
      });
    }
  }
  ...
  async function uploadConfig(): Promise<void> {
    ...
    if (!text) {
      dispatch({
        type: 'set-notification',
        notification: { kind: 'error', text: 'Paste a config first.' },
      });
      return;
    }
    ...
    if (isLegacyColorConfig(text)) {
      dispatch({
        type: 'set-pending-legacy',
        text: migrateLegacyColorConfig(text),
      });
      return;
    }
    ...
    <div className='tx-app'>
      ...
      <main className='tx-content'>
        ...
        {view === 'ambiguous' && state.variableStructure && (
          <MessageCard
            heading="Can't prepare automatically"
            onOpenGuide={() => bridge.openExternal(MIGRATION_GUIDE_URL)}
          >
          ...
          <MigrationPrompt
            ...
            onCancel={() =>
              dispatch({
                ...
                notification: {
                  kind: 'info',
                  text: 'Migration cancelled. The config was not changed.',
                },
              })
            }
          />
...
function PrepareGate({
  ...
  return (
    <section className='tx-message'>
      ...
      <p>
        This Figma file uses the old color structure with{' '}
        <code>Main color</code> and <code>Support color</code>. Importing would
        create a new <code>Color</code> collection alongside the old ones and
        break the file's bindings.
      </p>
      <p>
        Click <strong>Prepare variables</strong> to rename{' '}
        <code>Main color</code> to <code>Color</code> and clean up the variable
        names. The import then updates the existing variables instead of
        creating new ones.
      </p>
      ...
      <p className='tx-subtle'>
        After importing the updated variables you should finish the color
        migration (including cleaning up <code>Support color</code> in
        components).{' '}
