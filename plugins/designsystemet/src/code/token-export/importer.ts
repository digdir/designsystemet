...
export async function importToFigma(payload: ImportPayload): Promise<{ logs: string[] }> {
  const logs: string[] = []
  ...
  const collectionSpecs = buildCollectionSpecs(payload.preview, activeTokenSets, logs)
  ...
  const variableLookup = await syncVariables(collectionSpecs, collectionMap, logs)
  await syncTextStyles(payload.preview, activeTokenSets, variableLookup, fontCache, logs)
...
function buildCollectionSpecs(
  ...
  for (const mode of preview.themes) {
    ...
    const modeActiveTokenSets = getModeActiveTokenSets(preview, activeTokenSets, mode)
    ...
    for (const selected of mode.selectedTokenSets) {
      ...
      const tokens = preview.flatTokens.filter((token) => token.tokenSet === selected.tokenSet)

      for (const token of tokens) {
        const entries = expandVariableEntries(preview, mode.group, mode.name, token)
        ...
        for (const entry of entries) {
          ...
          if (!valueSpec) {
            logs.push(`Skipped unresolved value for ${mode.group}/${entry.name} (${mode.name})`)
            continue
          }
...
function inferVariableName(group: string, modeName: string, token: FlatToken): string {
...
function shouldResolveReferenceAsRaw(token: FlatToken, group: string): boolean {
  ...
  return group === COLLECTION.THEME && token.path.indexOf('border-radius.') === 0
}
...
function mapReferenceToVariableTarget(
  ...
  if (reference.indexOf('_size.') === 0 || reference.indexOf('size._') === 0) {
    const name = reference.indexOf('_size.') === 0
...
function convertRawVariableValue(type: string | null, value: unknown): VariableValue | null {
  switch (type) {
    case 'color':
...
async function syncCollections(
  ...
  for (const spec of specs) {
    const collection =
      collectionByName.get(spec.name) || figma.variables.createVariableCollection(spec.name)
...
async function ensureModes(
  ...
  if (existing.length === 1 && existing[0].name !== desiredModeNames[0]) {
    ...
    logs.push(`Renamed mode ${existing[0].name} -> ${desiredModeNames[0]} in ${collection.name}`)
  }
...
async function syncVariables(
  ...
  for (const spec of specs) {
    ...
    const variableByName = new Map(existingInCollection.map((item) => [item.name, item]))
  ...
  for (const spec of specs) {
    ...
    for (const desired of spec.variables.values()) {
      ...
      for (const [modeName, valueSpec] of desired.valuesByMode.entries()) {
        ...
        const modeId = modeIdByName.get(modeName)
        const targetVariable = byCompositeKey.get(`${valueSpec.collection}::${valueSpec.name}`)
        if (!modeId) {
          continue
        }
...
async function preloadAllFonts(
  ...
  for (const spec of specs) {
    ...
    for (const variable of spec.variables.values()) {
      ...
      for (const valueSpec of variable.valuesByMode.values()) {
        ...
        if (variable.name === 'font-family' || variable.name.endsWith('/font-family')) {
  ...
  for (const family of fontFamilies) {
    const allStyles = fontCache.availableFonts.filter((f) => f.fontName.family === family)
...
async function syncTextStyles(
  ...
  const desired = preview.flatTokens.filter(
    (token) => token.tokenSet === 'semantic/style' && token.type === 'typography',
  )

  const existing = await figma.getLocalTextStylesAsync()
  const desiredNames = new Set(desired.map((token) => token.path.replace(/\./g, '/')))

  for (const style of existing) {
    if (style.name.indexOf('typography/') === 0 && !desiredNames.has(style.name)) {
      style.remove()
  ...
  for (const token of desired) {
    ...
    const styleValue = resolveCompositeValue(token.value, preview, activeTokenSets) as Record<
      string,
      unknown
    > | null

    if (!styleValue) {
      logs.push(`Skipped text style ${styleName} because it could not be resolved`)
      continue
    }

    const fontFamily = typeof styleValue.fontFamily === 'string' ? styleValue.fontFamily : 'Inter'
    const fontWeight =
      typeof styleValue.fontWeight === 'string' ? styleValue.fontWeight : 'Regular'
    const fontName = await findFontName(fontCache, fontFamily, fontWeight)
    if (!fontName) {
      logs.push(`Skipped text style ${styleName} because font ${fontFamily} ${fontWeight} is unavailable`)
      continue
    }
    ...
    style.setBoundVariable('fontFamily', findVariable(variableLookup, COLLECTION.THEME, 'font-family'))
    ...
    style.setBoundVariable(
      'fontSize',
      typeof token.value === 'object' && token.value && 'fontSize' in token.value
        ? findVariable(
            variableLookup,
            COLLECTION.SIZE,
            normalizeFontSizeReference((token.value as Record<string, unknown>).fontSize),
          )
        : null,
    )
...
async function syncEffectStyles(
  ...
  const desired = preview.flatTokens.filter(
    (token) => token.tokenSet === 'semantic/style' && token.type === 'boxShadow',
  )
  ...
  for (const token of desired) {
    const styleName = token.path.replace(/\./g, '/')
    const resolved = resolveCompositeValue(token.value, preview, activeTokenSets) as
      | Array<Record<string, unknown>>
      | null

    if (!Array.isArray(resolved)) {
      logs.push(`Skipped effect style ${styleName} because it could not be resolved`)
      continue
    }
...
function resolveCompositeValue(
  ...
  if (Array.isArray(value)) {
    return value.map((item) => resolveCompositeValue(item, preview, activeTokenSets))
  }
...
async function ensureFontLoaded(fontCache: FontCache, fontName: FontName): Promise<void> {
...
function inferScopedThemeVariableName(preview: PreviewData, token: FlatToken): string | null {
  ...
  const knownThemeNames = new Set(preview.themeOptions.map((option) => option.name))
  const first = parts[0]
...
function getThemeNameFromTokenSet(preview: PreviewData, tokenSet: string): string {
  ...
  if (last === 'theme') {
    ...
    if (prev && prev !== 'light' && prev !== 'dark' && prev !== 'primary' && prev !== 'secondary') {
      return prev
    }
  }
