...
export function buildPreview(files: LoadedFile[]): PreviewData {
  ...
  const themesFile = validFiles.find((file) => file.path.endsWith('$themes.json'))
  const themes = Array.isArray(themesFile?.data) ? themesFile.data : []

  if (!themesFile) {
    warnings.push('Mangler $themes.json. Preview kan ikke gruppere modes fra token sets.')
  }
  ...
  const modePreviews = themes.map((theme) =>
    buildModePreview(theme as Record<string, unknown>, fileByTokenSet, warnings),
  )
  ...
  for (const tokenSet of missingTokenSets) {
    warnings.push(`Token set finnes i $themes.json men mangler fil: ${tokenSet}`)
  }
  ...
  const preview: PreviewData = {
    ...
    themes: modePreviews,
    collections,
    themeOptions: buildThemeOptions(modePreviews, tokenSets.map((set) => set.path)),
...
function buildModePreview(
  ...
  const selectedTokenSets = (theme.selectedTokenSets || {}) as Record<string, string>
  ...
  const sourceCount = selected.filter((item) => item.status === 'source').length
  if (sourceCount > 1) {
    warnings.push(`${theme.name}: har ${sourceCount} token sets markert som source.`)
  }
...
function inferVariablesForGroup(
  ...
  for (const mode of modes) {
    for (const selected of mode.selectedTokenSets) {
      ...
      const tokens = flatTokens.filter((token) => token.tokenSet === selected.tokenSet)
...
function buildThemeOptions(modePreviews: ModePreview[], tokenSetPaths: string[]): ThemeOption[] {
  const themeModes = modePreviews.filter((mode) => mode.group === COLLECTION.THEME)
...
function getSemanticColorScaleOrder(modePreviews: ModePreview[]): string[] {
  ...
  const standardOrder = ['neutral', 'success', 'warning', 'danger', 'error', 'info']
...
function appendModeNames(order: string[], modePreviews: ModePreview[], group: string): void {
  for (const mode of modePreviews) {
...
function compareColorScales(a: string, b: string, scaleOrder: string[]): number {
