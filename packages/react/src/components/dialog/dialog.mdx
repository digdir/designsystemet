...
Vår lukk knapp bruker `<form>`, som betyr at du ikke kan bruke den i andre `<form>`-elementer.
Du kan skru av vår lukk knapp ved å sette `closeButton={false}`. Da må du selv sørge for at dialogen kan lukkes, for eksempel ved å bruke `Dialog.close()` på en knapp du lager selv.
