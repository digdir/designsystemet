...
### `aria-describedby` og `aria-labelledby`

Dersom du ikke har tekst i trigger-elementer, setter vi automatisk `aria-labelledby`. Dersom det har tekst, setter vi `aria-describedby`. 
Du kan overstyre dette ved å bruke `type`-prop'en.
