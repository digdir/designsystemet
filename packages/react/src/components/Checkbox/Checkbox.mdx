...
Legg inn `allowIndeterminate: true` i `getCheckboxProps` for å opprette en overordnet avmerkingsboks som både kan velge og fjerne alle alternativer. Dette aktiverer en ekstra tilstand, `indeterminate`, ved siden av `checked` og `unchecked`. Den vises med en horisontal strek når én eller flere avmerkingsbokser er markert i gruppen. Den vises som en vanlig avmerkingsboks dersom alle i gruppen har samme tilstand.
...
### Skrivebeskyttede avmerkingsbokser

`Checkbox` støtter `readOnly`-attributtet for å gjøre feltet skrivebeskyttet, og gir en visuell indikasjon
som skiller seg fra redigerbare felter. Brukerne kan kopiere innholdet men ikke redigere det.
...
>
  <Do
    description='Plasser avmerkingsbokser vertikalt, slik at listen blir lettere å lese. '
    image='/img/component-guidelines/checkbox/CheckboxGroupV.png'
  />
  <Dont
    description='Du bør unngå å plassere avmerkingsbokser i flere kolonner.'
