...
Dersom du bruker `DropdownMenu.Trigger` er det innebyd tilgjengelighet for deg, men dersom du bruker `anchorEl` må du styre tilgjengelighet selv.
Da må du legge på `aria-expanded={true/false}` og `aria-haspopup='menu'`.
