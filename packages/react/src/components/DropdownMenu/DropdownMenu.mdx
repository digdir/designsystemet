...
Alternativt kan du bruke `anchorEl` for å styre hvor `DropdownMenu` skal vises, men da må du selv håndtere tilstand for åpning/lukking og attributter for tilgjengelighet.

_`anchorEl` vil bli fjernet i fremtiden, og vi anbefaler å bruke `DropdownMenu.Trigger` istedenfor._
...
Triggeren er en [Button](/docs/felles-button--docs) som standard.
