...
## Bruk
`Textarea` er en native komponent som kan kombineres med [`Field`](/docs/komponenter-field--docs) for å få utvidet funksjonalitet som prefix/suffix, beskrivelse og validering. [`Textfield`](/docs/komponenter-textfield--docs) med `multiline` er allerede sammensatt av disse komponentene, og er derfor anbefalt i de fleste tilfeller. Å bruke `Textarea` direkte passer best dersom du har behov for større kontroll over sammensetningen og ønsker å bygge noe mer skreddersydd.
...
`Textarea` bruker `.ds-input` klassen
