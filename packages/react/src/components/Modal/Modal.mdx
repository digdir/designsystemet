...
Dersom du bruker med `Trigger` komponent, vil modalen åpne med klikk på `Trigger` komponenten -
som betyr at du ikke trenger å bruke `ref`.
...
## Alternativ bruk med `ref`

Dersom du ikke bruker `Trigger` komponenten, så trenger du ikke å bruke `Modal.Root`.
...
### Med bruk av `ref` og ingen `Trigger`
