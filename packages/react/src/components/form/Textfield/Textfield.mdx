...
`Textfield` kalles tekstfelt på norsk. Det er et inndatafelt som gir brukerne mulighet til å skrive korte tekster eller tall.
...
- Ønsker du mer kontroll eller funksjonalitet, kan du bruke [`field`](/docs/komponenter-field--docs)-komponenten direkte.
...
Vi bruker `textfield` når vi vil gi brukeren mulighet til å skrive tekst eller korte svar på maksimalt én linje. Dette kan for eksempel være navn eller telefonnummer.

<br />

### Passer til

- å gi mulighet for korte tekster eller svar.
- å legge inn tall, for eksempel et telefonnummer.

### Passer ikke til 

- å gi lengre svar, bruk heller `textarea`.
- å legge inn formaterte data, som markdown.
...
Plassholdertekster forsvinner når brukerne skriver i feltet. Det er derfor bedre å inkludere hint og viktig informasjon i selve ledeteksten eller den tilhørende beskrivelsen. Kontrastkravene i WCAG gjelder for `placeholder` også, og da blir teksten så mørk at brukerne kan oppfatte `textarea` som utfylt.
