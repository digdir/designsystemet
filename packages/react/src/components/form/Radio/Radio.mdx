...
`Radio` skal alltid ha en `label`. Når du bruker ledetekten fra en annen plass, husk å bruke `aria-label` eller `aria-labelledby`.
...
Bruk `inline` for å plassere alternativene på samme linje. Anbefales ikke dersom du har mer enn to alternativer og lengre ledetekster.
...
<Canvas of={RadioGroupStories.Table} />
...
Radioknapper skal alltid ha en ledetekst. Den skal være så kort som mulig og hvert alternativ bør være formulert på samme måte. Radio-komponenten kan også ha hjelpetekst. Da kommer den rett under ledeteksten.
