...
Bruk `Fieldset` og `useRadioGroup` for gruppering av alternativknapper.

<Canvas of={RadioStories.Group} />

Du kan sende inn:

<Controls of={RadioStories.Group} include={['name', 'value', 'error', 'readOnly', 'disabled']} />

I respons får du objektet:

```ts
{
  getRadioProps: (value: string) => RadioProps, // Returnerer props du kan spre på <Radio /> eller <input type="radio" />
...
Vi ha opptil syv alternativer i en gruppe. Trenger vi å gi brukerne mer enn syv valg, bør vi heller bruke [`Combobox`](/docs/komponenter-combobox--docs)-komponenten. Hvis vi bare skal gi brukerne ett valg, kan en [`Switch`](/docs/komponenter-switch--docs) eller [`Checkbox`](/docs/komponenter-checkbox--docs) passe bedre.
...
Hvis vi bare har to valg med korte `labels`, kan vi vise dem ved siden av hverandre. I en [likert-skala](https://docs.altinn.studio/nb/app/development/ux/components/likert/) vil det også være naturlig å vise dem ved siden av hverandre. I en horisontal visning er det viktig at det kommer tydelig fram hvilken tekst som hører til hvilken boks.
...
En gruppe med alternativknapper bør alltid ha en `legend` og eventuelt en `description` dersom hjelpetekst er nødvendig.
