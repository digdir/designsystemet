...
Vi bruker `checkbox` (avmerkingsboks) til å gi brukerne en liste med valg, der de kan velge ett eller flere alternativer. Brukerne kan både velge og oppheve valgene de gjør. Vi anbefaler å bruke [`Checkbox.Group`](/docs/felles-checkbox-group--docs) for gruppering av flere valg.

Vær oppmerksom på:

- Ikke bruk `checkbox` når det er flere enn 7 alternativer.
- Vis listen vertikalt, slik at valgene presenteres i leseretningen.
...
- Hvis det er flere enn 7 svaralternativer bruk heller [`NativeSelect`](/docs/altinn-nativeselect--docs) eller `Combobox` (kommer snart).
...
<Stack direction="row" style={{
  ...
  <Do
    description='Checkboxes bør plasseres vertikalt, av hensyn til lesbarhet. '
...
<Dont
  description='Du bør unngå å plassere checkboxes i flere kolonner. For brukere som trenger å forstørre (zoome inn) nettsiden for å se godt nok, er en horisontal liste ekstra vanskelig.'
...
Plasser både (`legend`) og eventuell hjelpetekst (`description`) brukeren trenger for å forstå oppgaven, før listen med avkrysningsbokser kommer. I et skjema er ofte forklaringen formulert som et spørsmål. Vurder om du trenger å ha med informasjon om at brukeren kan velge flere svaralternativer.
...
Det skal være mulig å velge en `checkbox` kun ved å bruke tab-tasten, uten å klikke.
