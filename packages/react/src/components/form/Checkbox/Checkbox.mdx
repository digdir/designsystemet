...
Vi bruker `checkbox` (avmerkingsboks) til å gi brukerne en liste med valg, der de kan velge ett eller flere alternativer. Brukerne kan både velge og oppheve valgene de gjør. Bruk [`Checkbox.Group`](/docs/komponenter-checkbox-group--docs) til å gruppere flere valg.
...
Hvis det er flere enn 7 svaralternativer bruk heller [`NativeSelect`](/docs/komponenter-nativeselect--docs) eller [`Combobox`](/docs/komponenter-combobox--docs).

<br />

### Sortering

Alternativene skal som hovedregel sorteres alfabetisk. Velger du å presentere de mest aktuelle alternativene først kan det bli vanskelig å finne ønsket alternativ. Du kan også risikere å påvirke svaret som er uheldig.
...
Det finnes unntak, for eksempel i en [likert-skala](https://docs.altinn.studio/nb/app/development/ux/components/likert/). I en horisontal visning er det viktig at det kommer tydelig fram hvilken tekst som hører til hvilken boks.
...
- `label` skal stå til høyre for boksen. Hvis det er nødvendig, kan du ha en ekstra hjelpetekst (`description`) under `label` .
