...
Vi bruker `checkbox` (avmerkingsboks) til å gi brukerne en liste med valg, der de kan velge ett eller flere alternativer. Brukerne kan både velge og oppheve valgene de gjør. Vi anbefaler å bruke [`Checkbox.Group`](/docs/komponenter-checkbox-group--docs) til å gruppere flere valg.
...
- Hvis det er flere enn 7 svaralternativer bruk heller [`NativeSelect`](/docs/komponenter-nativeselect--docs) eller `Combobox` (kommer snart).
