...
```ts
{
  getCheckboxProps: (value: string | { multiple: true }) => CheckboxProps, // Returnerer props du kan spre på <Checkbox /> eller <input type="checkbox" />
...
return (
  <Fieldset>
    ...
    <Checkbox label='Alle' {...getCheckboxProps({ multiple: true })} />
