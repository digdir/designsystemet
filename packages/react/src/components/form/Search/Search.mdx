import { Meta, Canvas, Story, Controls, Primary } from '@storybook/blocks';
import { Information } from '../../../../../../docs-components';
import * as SearchStories from './Search.stories';

<Meta of={SearchStories} />

# Search

<Primary />
<Controls />

## Kontrollert

<Canvas of={SearchStories.Controlled} />

## Full bredde

<Canvas of={SearchStories.FullWidth} />

## Ikon søkeknapp

Bruker `searchButtonLabel` og `clearButtonLabel` til å sett innholdet i knappene på `Search`.
Dersom du ønsker å bruke kun et ikon på søkeknappen, sørg for at den har `title` definert.

<Canvas of={SearchStories.OnlyIcon} />

## Referanser

- [The ideal search box is 27 characters wide](https://www.smashingmagazine.com/2009/09/10-useful-usability-findings-and-guidelines/#6-the-ideal-search-box-is-27-characters-wide)
