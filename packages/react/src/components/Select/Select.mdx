...
# Select
`Select` er en nedtrekksliste som lar brukeren velge ett eller flere alternativer fra et statisk utvalg. Komponenten er bygget på HTML-elementet `select`, som benytter seg av operativsystemets standarder (derav 'native'). Dette sikrer forutsigbarhet og tilgjengelighet, men setter noen begrensninger på hvordan komponenten kan tilpasses, for eksempel ved at formateringen av alternativene ikke kan overstyres.
...
### Med feil
Når valget er påkrevd og brukeren forsøker å gå videre uten å ha gjort et valg, vises en feilmelding under `select`.
