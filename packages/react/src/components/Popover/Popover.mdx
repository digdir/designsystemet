...
# Popover
`Popover` vises over andre elementer i grensesnittet og er koblet til et spesifikt element. Den brukes til å vise tilleggsinformasjon, interaktive elementer eller korte forklaringer uten å navigere bort fra siden. `Popover` kan brukes som en utvidet løsning når `tooltips` ikke strekker til.
...
```
//  Tobias legger fram et eksempel her
```

<br/>

### Tilleggsinformasjon om et ord i en artikkel
Bruk `popover-link`-typografistilen (stiplet understrek) for å indikere at et ord har en forklaring. Popoveren kan inneholde en kort paragraf som utdyper ordets betydning.
...
Elementet som åpner popoveren bør bruke `aria-expanded="true/false"` for å indikere om popoveren er åpen eller lukket. Det er innebygd tilgjengelighet i Popover APIet i henhold til åpne/lukket tilstand og tastaturnavigasjon.
