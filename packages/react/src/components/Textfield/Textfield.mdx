...
## Bruk
`Textfield` er en samensatt komponent for [`input`](/docs/komponenter-input--docs) og [`textarea`](/docs/komponenter-textarea--docs). Du bytter mellom de ved å sette `multiline={true/false}`. `Textfield` har også innebygd [`Field`](/docs/komponenter-field--docs) som inkluderer `label`, `description`, `validationmessage`, `affix` og `counter`.
...
- Unngå placeholder-tekst - den forsvinner når brukeren begynner å skrive, og bør derfor ikke brukes som eneste forklaring. Bruk heller `Field.Description` under `label` hvis du vil gi eksempler eller forklaringer.
