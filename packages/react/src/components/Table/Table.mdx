...
Bruk bildetekst-elementet til å beskrive en tabell på samme måte som vi ville brukt en oevrskrift. 
En bildetekst/tabelltekst hjelper brukerne til å finne, navigere og forstå tabeller. 
Det er også andre stil-alternativer for tabelltekster.
...
Vi bruker standardstørrelsen medium på eksterne og interne flater.
...
Vi bruker table-komponenten når vi skal organisere og vise data på en strukturert måte i rader og kolonner for brukerne.  
...
For å spare plass, kan vi bruke meny hvis flere handlinger i en tabellrad ikke må være synlig for brukerne hele tiden. 
I overskiftceller bruker vi `<Table.HeaderCell>`, ikke `<Table.Cell>`. En celle er en overskrift hvis den beskriver innholdet i cellene under eller ved siden av.

Passer til å  
- gjøre det enkelt for brukerne å sammenligne og scanne informasjon 
- organisere og vise data på en effektiv måte i rekker og kolonner

Passer ikke til å 
- lage layout for innholdet på nettsiden eller å dele innholdet på en side systematisk 
...
For å gjøre det Størrelsen på overskriften i tabellen bør være lik størrelsen på innholdet i tabellen
- Når tabellen blir filtrert av ett eller flere skjemafelt, bruker vi en kompakt tabelloverskrift som passer til merkelappen for skjemafeltet.
- Når tabellen er rett under en overskrift 2 og et avsnitt med tekst, er det bedre å bruke en vanlig størrelse.
- Hvis du er usikker, kan du bruke en kompakt og bold overskrift. 
...
Alle tabeller må ha overskrifter for hver kolonne (`<Table.HeaderCell>`). Unntaket er kolonner som inneholder kun 
...
Hvis det er behov for å spare plass, kan vi skjule visuelt en eller flere overskrifter. 
