...
## Tilgjengelighet
Sørg for at du alltid bruker en `legend` for å sikre god tilgjengelighet for skjermlesere og andre hjelpemidler. Dersom `legend` kun er for skjermleser, bruk hjelpeklassen `ds-sr-only`, da sørger vi for at det ikke blir en unødvendig toppmarg på elemented direkte under `legend`.
