...
type GetStepsProps = Pick<PaginationProps, 'compact' | 'currentPage' | 'totalPages'>;

const getSteps = ({ compact, currentPage, totalPages }: GetStepsProps): ('ellipsis' | number)[] => {
...
export type UsePaginationProps = Pick<PaginationProps, 'compact' | 'totalPages'> &
  Partial<Pick<PaginationProps, 'currentPage'>>;

/** Hook to help manage pagination state */
export const usePagination = ({ totalPages, currentPage: currentPageProps = 1, compact }: UsePaginationProps) => {
