...
export const WithConfirmation = meta.story({
  render: () => {
    ...
    const [pending, setPending] = useState<(typeof options)[number] | null>(
      null,
    );

    function proposeSelection(option: (typeof options)[number] | null) {
      console.count('onSelectedChange');
      console.log('Proposed option:', option);
    ...
    return (
      <main style={{ maxWidth: 500, margin: '3rem auto' }}>
        <h1>Controlled Suggestion replays a rejected selection</h1>

        <p>
          Rejecting a proposed selection should leave the controlled value
          unchanged, but a second proposeSelection() is fired (see logs)
        </p>

        <Suggestion
          multiple={false}
          selected={selected}
          onSelectedChange={proposeSelection}
          filter={() => true}
        >
          <Suggestion.Input aria-label='Country' />
          <Suggestion.List>
            {options.map((option) => (
              <Suggestion.Option
                key={option.value}
        ...
        {pending && (
          <div>
            <p>
              Change from {selected.label} to {pending.label}?
            </p>
