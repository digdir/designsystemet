...
- `Button` skal ikke brukes for å navigere til andre sider eller ut av tjenesten, bruk heller [`link`](/docs/felles-link--docs).
- Unngå flere primærknapper per side. Brukerne kan bli usikre på hva de skal gjøre.
...
Vi har forskjellige knapper for å fremheve utvalgte handlinger. `Primary`, `secondary` og `tertiary` bestemmer hvilken viktighet handlingen har.
...
Bruk `asChild` dersom du trenger å bruke knappen som en lenke. Vurder først om det ikke holder å bruke [`Link`](/docs/felles-link--docs) komponenten.
...
Knapper med kun ikon bruker vi bare for godt kjente ikoner som Lukk og Slett. Når vi bruker knapper med kun ikon, må vi huske en tilgjengelig tekst som forklarer hva knappen gjør. Dette kan legges i `title`, `aria-label` eller i en `Tooltip` som i eksempelet under.

<Canvas of={ButtonStories.BareIkon} />

### Knapp med tekst og ikon

Når vi bruker ikon sammen med tekst, skal ikonet ha `aria-hidden` slik at teksten ikke leses opp av skjermleser to ganger.

<Canvas of={ButtonStories.TekstOgIkon} />

Vi plasserer ikonet til venstre eller høyre for teksten avhenger av hva knappen kommuniserer og hvor den plasseres i grensesnittet.
...
I eksempelet over har vi brukt `aria-disabled` til å deaktivere knappene. Dette er den anbefalte metoden for å vise at en knapp er deaktivert, da det gjør at den fremdeles kan fokuseres på, f.eks. ved navigasjon med tastatur. Slik vil blant annet skjermlesere og andre hjelpemidler bli informert om at knappen finnes, men ikke er aktiv.
...
Vi skal tydelig beskrive handlingene knappene utfører. Vi skal ikke ha flere handlinger i samme knapp.
...
<Stack direction="row" style={{
  ...
  <Dont
    description='Flere handlinger på samme knapp bør unngås'
...
Vi skal holde oss til en primærknapp per side.
...
<Stack direction="row" style={{
  ...
  <Do
    description='Det skal kun være èn primærknapp per side'
...
<Stack direction="row" style={{
  ...
  <Do
    description='Bruk kun et ikon'
    image='/img/component-guidelines/button/button-ex1-do.png'
  />

  <Dont
    description='Ikke bruk flere ikoner i samme knapp'
