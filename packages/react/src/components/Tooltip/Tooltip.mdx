...
`Tooltip` har to fargevarianter tilpasset `light` og `dark` modus.
...
### Markering av tekst
Hvis `tooltip` er knyttet til et ord i en tekst, marker ordet med en stiplet linje under. Bruk dette med måte, da for mye markering `hover`-innhold kan virke forstyrrende for brukeren.
...
På berøringsskjermer er `tooltip` mindre egnet, fordi de vanligvis aktiveres ved `hover` eller `fokus`, som ikke støttes på disse enhetene. `Tooltip` vises i stedet kun når brukeren trykker på elementet, og forsvinner igjen når brukeren trykker utenfor elementet eller `tooltip` areal.
