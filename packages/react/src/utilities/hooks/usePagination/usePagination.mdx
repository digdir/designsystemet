import { Meta, Primary, ArgTypes, Controls } from '@storybook/blocks';

import * as Stories from './usePagination.stories';
import { usePagination } from '../';

<Meta of={Stories} />

# usePagination

En react hook for å håndtere paginering.

<Primary />
<Controls of={Stories.Preview} />

## Props

<ArgTypes of={Stories.Props} include={['currentPage', 'totalPages', 'onChange', 'setCurrentPage', 'showPages']} />

<br />
I respons får du objektet:

```ts
{
  hasPrev: boolean, // Indikererer om det finnes en forrige side
  hasNext: boolean, // Indikererer om det finnes en neste side
  prevButtonProps: PaginationButtonProps, // Properties du kan spre på forrie-knappen
  nextButtonProps: PaginationButtonProps, // Properties du kan spre på neste-knappen
  pages: Array<{
    page: number | string; // Tall for side, eller "ellipsis" streng dersom knappen skal være ...
    itemKey: string; // key du kan sende til Pagination.Item for å sikre riktig fokus håndtering
    buttonProps: PaginationButtonProps; // Properties du kan spre på Pagination.Button
  }>
}
```

## Bruk

```tsx
const onChange = (event, page: number) => console.log(`Going to page ${page}`);
const [currentPage, setCurrentPage] = useState(4);
const { pages, prevButtonProps, nextButtonProps, hasNext, hasPrevious } = usePagination({
  currentPage,
  setCurrentPage,
  onChange,
  totalPages: 10,
  showPages: 7,
});
```

### På `Pagination.Button`

Du looper gjennom `pages` fra respons-objektet, og sender `buttonProps` til `Pagination.Button`.
Legg merke til at vi sjekker `typoeof` på `page`. Dette er fordi ellipser skal være en tom `li`.

```tsx
{pages.map(({ page, itemKey, buttonProps }) => (
  <Pagination.Item key={itemKey}>
    {typeof page === 'number' && (
      <Pagination.Button
        asChild
        aria-label={`Side ${page}`}
        {...buttonProps}
      >
        <a href={`#side-${page}`}>{page}</a>
      </Pagination.Button>
    )}
  </Pagination.Item>
))}
```
