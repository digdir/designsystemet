...
#### `ds-readonly-icon`
Legger til et hengelås-ikon via `::before` til elementet klassen settes på. Brukes blant annet i `Field` på label når input er `readonly`. <span className='ds-readonly-icon'>Eksempel</span>.
...
Legger til designsystemets fokusring rundt elementet. <span className='ds-focus--visible'>Eksempel.</span>
