...
#### `ds-readonly-icon`
Legger til et hengelås-ikon via `::before` til elementet klassen settes på. Brukes blant annet i `Field` på label når input er `readonly`.
...
Legger til designsystemets fokusring rundt elementet.
