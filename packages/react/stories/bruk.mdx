...
Dette betyr at du ikke kan skrive `<Details.Item>`, men må skrive `<DetailsItem>`. Grunnen for dette er at serveren ikke kan få tak i noe
...
<CodeSnippet language='ts'>
{`
import { Details, DetailsHeading, DetailsItem, DetailsContent } from '@digdir/designsystemet-react';
...
Details har f.eks en `Details` komponent som må wrappes rundt `Details.Item` komponentene.
