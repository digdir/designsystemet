...
export function apply(opts?: PopoverPolyfillOptions) {
  ...
  Object.defineProperties(HTMLElement.prototype, {
    ...
    showPopover: {
      enumerable: true,
      configurable: true,
      writable: true, // HTML prototype methods are writable, so the polyfills methods should be too
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      value(_options: PopoverShowPopoverOptions = {}) {
        showPopover(this);
      },
    },
  ...
  const onKeydown = (event: Event) => {
    ...
    if (
      ...
      hideAllPopoversUntil(document, true, true);
    }
  };
