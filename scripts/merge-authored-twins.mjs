...
const AUTHORED = JSON.parse(readFileSync(join(ROOT, 'scripts/twins-authored.json'), 'utf8'));
...
for (const [slug, entry] of Object.entries(AUTHORED)) {
  ...
  if (!existsSync(twinPath)) {
    console.warn(`  skip ${name}: no generated twin (run generate-twins.mjs first)`);
    continue;
  }
  ...
  for (const key of ['a11y', 'relations', 'composition']) {
    for (const rule of fields[key] ?? []) {
      ...
      const found = SUBPAGES.find((sub) => quotes.every((q) => pages[sub].includes(q)));
      rule.verified = found !== undefined;
      rule.sourcePage = found ? docsUrl(slug, found) : null;
      if (!rule.verified) {
        allVerified = false;
        reviewRows.push([name, key, rule.rule ?? `${rule.component}: ${rule.note ?? ''}`, rule.quote ?? '']);
      }
    }
  }

  // Stamp after verification, so no field can claim a review the twin loses later.
  const method = allVerified && reviewedAgainst
...
const sheet = [
  ...
  ...(reviewRows.length
    ? ['| Component | Field | Rule | Quote |', '|---|---|---|---|',
       ...reviewRows.map((r) => `| ${r.map((c) => String(c).replace(/\|/g, '\\|').slice(0, 160)).join(' | ')} |`)]
...
console.log(`merged ${merged} components: ${reviewed} reviewed, ${reviewRows.length} rules pending review`);
